...
export default function EmployeesClient() {
  ...
  const toggleStatus = (id: number) => {
    setEmployees((prev) =>
      prev.map((e) =>
        e.id === id ? { ...e, status: e.status === "active" ? "leave" : "active" } : e
      )
    );
  };

  const addEmployee = (form: typeof EMPTY_FORM) => {
    ...
    <>
      ...
      <div className="rules-header">
        <div>
          ...
          <p className="rules-subtitle">등급 클릭으로 변경 · 직원 추가 및 상태 관리</p>
        </div>
      ...
      <div className="emp-table-wrap">
        ...
        <div className="emp-table-header">
          ...
          <span className="emp-col-role">직무</span>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((emp, i) => (
              <motion.div
                ...
                animate={{ opacity: deleteId === emp.id ? 0 : 1, x: deleteId === emp.id ? 20 : 0 }}
              ...
              >
                ...
                <div className="emp-col-name emp-name-cell">
                  <div className="emp-avatar" style={{ background: GRADE_META[emp.grade].bg, color: GRADE_META[emp.grade].color }}>
                    {emp.name[0]}
                  </div>
                ...
                <div className="emp-col-grade">
                  <GradeBadge grade={emp.grade} onClick={() => cycleGrade(emp.id)} />
                </div>
